...
namespace MISA.WebFresher2023.Demo.DL.Repository
{
    public class EmployeeRepository : BaseRepository<Employee>, IEmployeeRepository
    {
        ...
        // <summary>
        /// Hàm kiểm tra account này có trong bookkeeping không 
        /// </summary>
        /// <param name="id">Id của account</param>
        /// <returns>bool</returns>
        /// Author: LeDucTiep (14/07/2023)
        public async Task<bool> CheckIsInReceiptAsync(Guid id)
        {
            ...
            {
                // Lenh truy van
                string sql = "SELECT EXISTS (SELECT * from receipt r WHERE r.EmployeeId = @id)";
        ...
        /// <returns>bool</returns>
        /// Author: LeDucTiep (14/07/2023)
        public async Task<bool> CheckIsInCustomerAsync(Guid id)
        {
            // Kết nối với database
            var connection = await _msDatabase.GetOpenConnectionAsync();
            try
            {
                // Lenh truy van
                string sql = "SELECT EXISTS (SELECT * from customer c WHERE c.EmployeeId = @id)";
