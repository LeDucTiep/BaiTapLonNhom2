...
namespace MISA.WebFresher2023.Demo.DL.Repository
{
    public class AccountRepository : BaseRepository<Account>, IAccountRepository
    {
        ...
        /// <returns>bool</returns>
        /// Author: LeDucTiep (14/07/2023)
        public async Task<bool> CheckIsInBookKeepingAsync(Guid id)
        {
            // Kết nối với database
            var connection = await _msDatabase.GetOpenConnectionAsync();
            try
            {
                // Lenh truy van
                string sql = "SELECT EXISTS (  SELECT * from bookkeeping b   WHERE b.DebitAccountId = @id  OR b.CollectAccountId = @id)";
