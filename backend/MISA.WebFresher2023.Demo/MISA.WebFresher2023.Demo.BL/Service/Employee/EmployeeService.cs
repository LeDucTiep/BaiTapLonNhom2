...
namespace MISA.WebFresher2023.Demo.BL.Service
{
    ...
    public class EmployeeService : BaseService<Employee, EmployeeDto, EmployeeCreateDto, EmployeeUpdateDto>, IEmployeeService
    {
        ...
        public override async Task<List<int>> DeleteValidate(Guid id)
        {
            ...
            bool isInCustomer = await _employeeRepository.CheckIsInCustomerAsync(id);

            if (isInCustomer)
            {
                errorList.Add((int)EmployeeErrorCode.CantDeleteEmployeeInCustomer);
            }
            // Liên quan phiếu thu
            bool isInReceipt = await _employeeRepository.CheckIsInReceiptAsync(id);

            if (isInReceipt)
            {
                errorList.Add((int)EmployeeErrorCode.CantDeleteEmployeeInReceipt);
            }
            return errorList;
        }
